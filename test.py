...
model_name = 'best_weights_original.hdf5'
model_name = 'best_weights.hdf5'

model_name_full = os.path.join('weights', model_name)

model.load_weights(filepath=model_name_full)


print(input_size)


test_filenames = glob.glob("MyDataset/test/*.jpg")
...
for start in tqdm(range(0, len(test_filenames), batch_size)):
    ...
    for id in ids_test_batch:
        img = cv2.imread('MyDataset/test/{}.jpg'.format(id))
    ...
    for index, pred in enumerate(preds):
        ...
        cv2.imwrite('MyDataset/test/segmentation/{}.png'.format(id), prob)
